...
export default function TechStack() {
  const [randomRotations, setRandomRotations] = React.useState<number[]>([]);

  React.useEffect(() => {
    setRandomRotations(techStack.map(() => Math.random() * 6 - 3));
  }, []);
    ...
    <section className="py-16 relative">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
        {techStack.map((tech, index) => (
          <motion.div
            key={tech.name}
            initial={{ opacity: 0, y: 20, rotate: randomRotations[index] || 0 }}
            whileInView={{ opacity: 1, y: 0 }}
